...
Spring allows for the no--arg constructor of a Bean to have an access level stricter than public.
...
This means that the Hibernate interface, and engine, is much more mature and stable than the OGM implementation.
...
The ability to scale out, as well as of up, was also an important factor.


\subsubsection{Flyway}
Flyway is a lightweight database migration tool.
Meaning that is applies database migration scripts in sequence, remembering the current point in the sequence between runs.
