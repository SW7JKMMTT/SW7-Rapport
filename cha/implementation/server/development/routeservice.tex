...
We have previously documented how the relationship between \code{VehicleData} and \code{VehicleDataPoint}s in \cref{ssec:miscdata}.
We do not use a builder to construct the \code{VehicleDataPoint}s, but instead we use annotations which are part of Jackson, which we described in \cref{subsec:serialization_tech}.
This is done since it is easier to add more different \code{VehicleDataPoints}, since they would only require changing the model, which we described in \cref{ssec:miscdata}.
