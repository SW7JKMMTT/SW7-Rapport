\section{Testing the Server}\label{sec:testing}
...
Furthermore testing is a part of the requirements of the server, as mentioned in \cref{sec:requirements_elicitation}\fxnote{Add testing som requirement}.
...
At line 1, the annotation \code{@Test} is JUnits way of define a method as a test.
As can be seen in \cref{lst:unit_test_example}, we use the \ac{AAA} pattern when doing unit tests.\fxnote{Kan det ikke godt passe er det kun er ved unit tests AAA bliver brugt, og ikke ved integration tests?}
...
\begin{listing}
    ...
    \end{java2}
    \caption{An example of a unit test from \code{TestRoute.java} which is a part of Core.}
