...
This works for testing and proving that the server works as intended, but the server also relies on the producer client.
...
For the server specifically as that is our focus, it would require a design that allows us to store data for a variety of fields without storing unwanted information, e.g. cargo weight for a city bus.
