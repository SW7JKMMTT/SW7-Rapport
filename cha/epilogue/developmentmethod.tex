...
We decided to use build automation, and continuous integration, in order to automatically verify that our changes were working and did not break existing functionality.
...
The constructed system is testable, and has numerous tests, which run automatically on every commit, and after every build, which we found greatly lowered the amount of errors on the production server.
To further our goal of proper testing, we placed a build monitor in the work space, which we also had a positive effect on stability and error rate.
%Systematic Integration tests(JUnit) not interesting if no interaction with DB, thus Integration > Unit
%Load testing, system test(testing all services) JMeter
\subsubsection{Build Automation}
...
The system tests, test that the API acts as the API specification describes it, i.e. as the Enunciate documentation described.%I assume we have explained Enunciate
...
Most notably we introduced indices to the SQL database, and avoided fetching the entire list of waypoint/miscdata when adding one of them.  
...
Our use of Jenkins CI to achieve our goal of test focus, and build automation, was successful.
We wrote a lot of unit tests, since we felt the impact of running them for every build, and change.
Our tests found some errors, which would have otherwise made it into production, and might have disrupted the production server.
