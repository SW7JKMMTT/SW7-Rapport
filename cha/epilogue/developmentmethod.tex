...
Ideally we wanted to reduce the overhead from Scrum and go back to using only a few elements from Scrum, while maintaining some of the additional processes which we had used.
%%%%%%%%%%%
...
For this semester we decided that using Scrum with sprints and sprint meetings would be too much overhead, as we simply wanted utilise some of the management tools we had successfully used in the past.
...
Using Git we also used naming conventions for our branches as to identify them, we have three types of branches, release, development and bugfix.
At all times a working release branch must exist, the branch should be buildable to all times.
A development branch is for new features and a bugfix branch is for fixing bugs.
Whenever something is completed in a development or bugfix branch, and should be pushed to the release branch, it goes through a review process.

The review process helps reduce the amount of times something would be iterated upon, i.e. number of bugfix branches we would create.
...
The code written  6\textsuperscript{th} semester would become a codebase for new students at a later point, which is why we chose to be so strict and rigid with out review.
