...
For our particular system we have two types of distinct clients communicating with the server.
\begin{description}
...
Ignoring this does remove some load scalability options which will be covered later in the implementation section \fxnote{opdater når denne section eksistere}.
...
As an example a consumer client could use a layered architecture.
...
\end{itemize}
For this example the fourth layer, system support, could either simply be a layer that handles communication with the server we provide, or there could be a fifth layer that does that, and then stores data locally to reduce the number of requests.
For the consumer essentially any arbitrary architecture could be chosen, and the chose would depend on the specific need and use case the client has.

\subsection{Requirements fulfillment(working title)}

%A test section written by the one and only Jesper Jensen should follow here.
