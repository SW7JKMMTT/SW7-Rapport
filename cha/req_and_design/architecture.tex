...
In order to attain device scalability the format communicated over must be neutral.
...
Due to the communication choice being our decision, simply deciding to use a device neutral format such as JSON will make the system support device scalability.
...
In our case version scalability is tied heavily to functional scalability, as it for the server would be expanding what information it can receive.
...
This results in both version and functional scalability really depending on the server's internal architecture, which happens to be a layered architecure which faciliates low coupling and through low coupling functional scalability.
